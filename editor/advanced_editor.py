...
def edit_3_4_custom_layout_template(input_path: str, logo_path: str, output_path: str, headline: str = "FUNNY MEME!", story: str = "", source_credit: str = "", safety_actions: list = None):
    ...
    # Scale video to fit inside 1080x1080 area and center overlay onto frame
    filter_complex = (
        f"[0:v]{flip_filter}setpts=PTS/1.05,scale=1080:1080:force_original_aspect_ratio=decrease,eq=contrast=1.05:brightness=0.02:saturation=1.15:gamma=1.0,unsharp=5:5:0.5[vid_processed];"
        "[1:v][vid_processed]overlay=(main_w-overlay_w)/2:500+(1080-overlay_h)/2[outv]"
    )
...
def process_video_dynamically(input_path: str, logo_path: str, output_path: str, task: dict = None):
    ...
    headline = analysis.get("short_headline", "FUNNY MEME!")
    ...
    message_text = (
        f"🎬 **EDITING REPORT**\n\n"
        f"**Workflow Name:** Funny Reels Pipeline\n"
